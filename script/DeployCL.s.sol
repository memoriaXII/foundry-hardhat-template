...
import {GaugeV2} from "contracts/gauge/GaugeV2.sol";
...
import {QuoterV2} from "contracts/lens/QuoterV2.sol";
...
contract DeployCL is Script {
    ...
    CLPoolFactory public poolFactory;
    NonfungibleTokenPositionDescriptor public nftDescriptor;
    NonfungiblePositionManager public nft;
    GaugeV2 public gaugeImplementation;
    GaugeV2Factory public gaugeFactory;
    ...
    function run() public {
        ...
        factoryRegistry = abi.decode(
            vm.parseJson(jsonConstants, ".FactoryRegistry"),
            (address)
        );
        poolFactoryOwner = abi.decode(
            vm.parseJson(jsonConstants, ".poolFactoryOwner"),
            (address)
        );
        feeManager = abi.decode(
            vm.parseJson(jsonConstants, ".feeManager"),
            (address)
        );
        notifyAdmin = abi.decode(
            vm.parseJson(jsonConstants, ".notifyAdmin"),
            (address)
        );
        factoryV2 = abi.decode(
            vm.parseJson(jsonConstants, ".factoryV2"),
            (address)
        );
        nftName = abi.decode(vm.parseJson(jsonConstants, ".nftName"), (string));
        nftSymbol = abi.decode(
            vm.parseJson(jsonConstants, ".nftSymbol"),
            (string)
        );
        ...
        // deploy gauges
        gaugeImplementation = new GaugeV2();
        //address(this)
        gaugeFactory = new GaugeV2Factory({
            _initialOwner: address(this),
            _gaugeCLImplementation: address(gaugeImplementation),
            _nonFungiblePositionManager: address(nft),
            _voter: voter
        });
        ...
        vm.writeJson(
            vm.serializeAddress("", "PoolFactory", address(poolFactory)),
            path
        );
        vm.writeJson(
            vm.serializeAddress(
        ...
        vm.writeJson(
            vm.serializeAddress("", "NonfungiblePositionManager", address(nft)),
            path
        );
        vm.writeJson(
            vm.serializeAddress(
        ...
        vm.writeJson(
            vm.serializeAddress("", "GaugeFactory", address(gaugeFactory)),
            path
        );
        ...
        vm.writeJson(
            vm.serializeAddress("", "SwapRouter", address(swapRouter)),
            path
        );
    }
